...
const loadPlaywright = () => {
  ...
  const requireFromComponentTester = createRequire(path.join(componentTesterDir, "package.json"));
...
const waitForActiveWorkspaceTab = async (workspaceTabsEndpoint, tab, timeoutMs) => {
  ...
  throw new Error(`Timed out waiting for workspace tab '${tab}' to become active.`);
};
...
const resolveInstance = async (options) => {
  if (options.httpPort) {
    ...
    const instance = normalizeBloomInstanceInfo(response.json, options.httpPort);
  ...
  if (options.runningBloom) {
    ...
    if (!instance) {
      throw new Error("No running Bloom instance was found on Bloom's standard HTTP port range.");
    }
...
const getBloomPage = (browser) => {
  ...
  return pages.find(
    (candidate) =>
      candidate.url().includes("/bloom/") && !candidate.url().startsWith("devtools://"),
  );
};
...
const clickWorkspaceTab = async (page, tab) => {
  ...
  const mainTabsButton = page.locator("#main-tabs button").filter({ hasText: tabLabel });
...
const main = async () => {
  ...
  if (!instance.cdpPort) {
    throw new Error("The selected Bloom instance did not report a CDP endpoint.");
  }
  ...
  try {
    ...
    if (!page) {
      throw new Error(`Could not find a Bloom WebView2 target on ${cdpEndpoint}.`);
    }

    await page.waitForLoadState("domcontentloaded");
    const clickSurface = await clickWorkspaceTab(page, tab);

    const tabs = await waitForActiveWorkspaceTab(workspaceTabsEndpoint, tab, options.timeoutMs);
    const bodyClassName = await page.locator("body").evaluate((element) => element.className);
