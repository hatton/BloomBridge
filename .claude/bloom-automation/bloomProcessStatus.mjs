...
const parseArgs = () => {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (arg.startsWith("--http-port=")) {
      options.httpPort = requireTcpPortOption("--http-port", arg.slice("--http-port=".length));
      continue;
    }
  }
...
const runningBloomInstances = options.runningBloom ? await findRunningStandardBloomInstances() : [];
...
if (options.httpPort) {
  ...
  if (instanceInfo.reachable && instanceInfo.json) {
    selectedRunningBloomInstance = normalizeBloomInstanceInfo(instanceInfo.json, options.httpPort);
  }
...
if (selectedRunningBloomInstance?.processId) {
  ...
  if (processRecord) {
    ...
    const detectedRepoRoot = processChain.find((entry) => entry.repoRoot)?.repoRoot;
    ...
    selectedRunningBloomProcess = {
      ...
      matchesExpectedRepoRoot:
        !!detectedRepoRoot &&
        detectedRepoRoot.toLowerCase() === processState.expectedRepoRoot?.toLowerCase(),
      processChain,
    };
  }
}
...
const result = {
  ...
  isRunning: selectedRunningBloomInstance ? true : processState.bloomProcesses.length > 0,
...
if (selectedRunningBloomInstance) {
  console.log(`Selected Bloom HTTP port: ${selectedRunningBloomInstance.httpPort}`);
  console.log(`Selected Bloom CDP port: ${selectedRunningBloomInstance.cdpPort || "unknown"}`);
  console.log(`Selected Bloom PID: ${selectedRunningBloomInstance.processId || "unknown"}`);
}
console.log(`Running from expected repo root: ${result.runningFromExpectedRepoRoot}`);
console.log(`Running from different repo root: ${result.runningFromDifferentRepoRoot}`);
console.log(`Workspace tabs endpoint reachable: ${result.endpoints.workspaceTabs.reachable}`);
...
for (const bloomProcess of result.bloomProcesses) {
  ...
  console.log(`Detected repo root: ${bloomProcess.detectedRepoRoot || "unknown"}`);
  console.log(`Matches expected repo root: ${bloomProcess.matchesExpectedRepoRoot}`);
