...
export const requireTcpPortOption = (optionName, value) => {
  ...
  if (!port) {
    throw new Error(`${optionName} must be an integer from 1 to 65535. Received: ${value}`);
  }
...
export const getStandardBloomHttpPorts = () =>
  Array.from(
    ...
    (_, index) => standardBloomStartingHttpPort + index * standardBloomReservedPortBlockLength,
  );
...
export const extractRepoRoot = (text) => {
  ...
  const exeMatch = normalized.match(/([A-Za-z]:\\[^"\r\n]+?)\\output\\[^"\r\n]+?\\Bloom\.exe/i);
  if (exeMatch?.[1]) {
...
export const getWindowsProcessSnapshot = () => {
  const rawProcesses = [...queryProcessesByName("Bloom.exe"), ...queryProcessesByName("dotnet.exe")]
    .map((record) => ({
  ...
  const byId = new Map(rawProcesses.map((record) => [record.processId, record]));
...
export const buildProcessChain = (processRecord, byId) => {
  ...
  for (let i = 0; i < 8 && current; i++) {
    chain.push({
      ...
      repoRoot: extractRepoRoot(current.executablePath) || extractRepoRoot(current.commandLine),
    });
  ...
  const toRecord = (processRecord) => {
    ...
    return {
      ...
      matchesExpectedRepoRoot:
        ...
        detectedRepoRoot.toLowerCase() === normalizedExpectedRepoRoot.toLowerCase(),
      processChain,
    };
  };
...
export const findRunningStandardBloomInstances = async () => {
  ...
  return responses
    .filter(({ instanceInfo }) => instanceInfo.reachable && !!instanceInfo.json)
    .map(({ port, instanceInfo }) => normalizeBloomInstanceInfo(instanceInfo.json, port))
