...
const parseArgs = () => {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (arg === "--port") {
      options.port = requireTcpPortOption("--port", requireOptionValue(args, i, "--port"));
      i++;
      continue;
    }

    if (arg.startsWith("--port=")) {
      options.port = requireTcpPortOption("--port", arg.slice("--port=".length));
      continue;
    }
...
const printText = (result) => {
  ...
  if (result.targets.length === 0) {
    ...
    console.log("Try rerunning with --all if Bloom is open but did not match the default filter.");
    return;
  }
  ...
  while (true) {
    try {
      if (options.httpPort) {
        ...
        selectedInstance = normalizeBloomInstanceInfo(instanceInfo.json, options.httpPort);
        ...
        if (!runningBloomInstance.cdpPort) {
          throw new Error("The running Bloom instance did not report a CDP endpoint.");
        }
      ...
      const normalizedTargets = targets.map((target) => ({
        ...
        devtoolsFrontendUrl: normalizeFrontendUrl(origin, target.devtoolsFrontendUrl),
        likelyBloomTarget: isLikelyBloomTarget(target),
      }));

      filteredTargets = options.all
        ? normalizedTargets
        : normalizedTargets.filter((target) => target.likelyBloomTarget);

      if (!options.wait || filteredTargets.length > 0 || Date.now() >= deadline) {
        break;
      }
    } catch (error) {
      lastError = error;
