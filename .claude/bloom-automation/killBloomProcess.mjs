...
const exactTargetRequested = !!options.httpPort || !!options.pid || !!options.watchPid;
...
if (options.httpPort) {
  ...
  if (instanceInfo.reachable && instanceInfo.json) {
    targetedInstance = normalizeBloomInstanceInfo(instanceInfo.json, options.httpPort);
...
if (processIds.size > 0) {
  ...
  for (const requestedProcessId of [...processIds]) {
    ...
    for (const chainEntry of processChain) {
      if (
        ...
        (chainEntry.name === "dotnet.exe" && chainEntry.commandLine?.includes("BloomExe.csproj"))
      ) {
  ...
  const bloomProcesses = processState.bloomProcesses.filter(
    (processRecord) => !options.onlyMismatched || !processRecord.matchesExpectedRepoRoot,
  );
...
if (exactTargetRequested && requestedProcessIds.length === 0) {
  console.log(exactTargetResolutionError || "No explicit Bloom process target could be resolved.");
  process.exit(1);
}
