...
export const optionsSchema: OptionSpec[] = [
  ...
  },
  {
    ...
    default: "busy",
    choices: [
      { value: "covers", label: "Only image covers" },
      { value: "busy", label: "Image covers + pages too busy to convert well" },
      { value: "anyCanvas", label: "Image covers + any page with text over a picture" },
      { value: "all", label: "All pages (maximum fidelity)" },
    ],
    stage: "html",
    help: "Without a human touching things up, this converter can't always reproduce a complex page exactly using editable text boxes. For which pages should it just snapshot the original PDF page instead? 'covers' rebuilds every interior page as editable text; 'busy' (default) additionally snapshots pages too busy to convert well; 'anyCanvas' snapshots any page with text over a picture; 'all' snapshots every page for maximum fidelity (nothing editable or translatable; only a few pages are OCR'd for metadata/languages).",
  },
  {
    key: "target",
    cliFlag: "--target",
